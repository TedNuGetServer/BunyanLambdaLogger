...
public class LambdaLoggerOptions
{
  ...
  private const string DEFAULT_SECTION_NAME = "Lambda.Logging";
    ...
    : this()
  {
    ...
    if (TryGetString(loggerConfiguration, INCLUDE_CATEGORY_KEY, out var includeCategoryString))
    {
      IncludeCategory = bool.Parse(includeCategoryString);
    }

    if (TryGetString(loggerConfiguration, INCLUDE_LOG_LEVEL_KEY, out var includeLogLevelString))
    {
      IncludeLogLevel = bool.Parse(includeLogLevelString);
    }

    if (TryGetString(loggerConfiguration, INCLUDE_NEWLINE_KEY, out var includeNewlineString))
    {
      IncludeNewline = bool.Parse(includeNewlineString);
    }

    if (TryGetSection(loggerConfiguration, LOG_LEVEL_KEY, out var logLevelsSection))
    {
  ...
  private static Func<string, LogLevel, bool> CreateFilter(IConfiguration logLevelsSection)
  {
    ...
    foreach (var logLevel in logLevels)
    {
      ...
      if (!Enum.TryParse(minLevelValue, out LogLevel minLevel))
      {
    ...
    return (category, logLevel) =>
    {
      ...
      if (logLevelsMapping.TryGetValue(category, out var minLevel))
      {
